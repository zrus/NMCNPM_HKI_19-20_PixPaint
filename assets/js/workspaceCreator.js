...
function elt_append(parent, type, props, ...children) {
  let dom = document.createElement(type);
  if (props) Object.assign(dom, props);
  for (let child of children) {
    if (typeof child != "string")
...
class PictureCanvas {
  ...
  syncState(picture) {
    ...
    drawPicture(this.picture, this.dom, scale, null);
  }
}
  ...
  syncState(state, canvas) {
    ...
    for (let y = 0; y < state.picture.height; y++) {
      for (let x = 0; x < state.picture.width; x++) {
          if (flag == true) cx.fillStyle = "#f0f0f0";
...
class ColorSelect {
  constructor(state, { dispatch }) {
    ...
    this.dom = elt_append("tools-container", "label", null, "🎨 Color: ", this.input);
    //document.getElementById("tools-container").appendChild(elt("label", null, "🎨 Color: ", this.input));
    document.getElementById("tools-container").appendChild(elt("br"));
  }
...
class SaveButton {
  constructor(state) {
    ...
    // document.getElementById("tools-container").appendChild(elt("button", {
    this.dom = elt_append("tools-container", "button", {
      onclick: () => this.save()
    }, "💾 Save");
  }
  save() {
