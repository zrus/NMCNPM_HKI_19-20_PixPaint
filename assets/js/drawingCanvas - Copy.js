...
const startState = {
  tool: "draw",
  color: "#000000",
  picture: Picture.empty(500, 500, "#f0f0f0"),
  done: [],
  doneAt: 0
};
...
function startPixelEditor({state = startState, tools = baseTools, controls = baseControls}) {
  let app = new PixelEditor(state, { tools, controls, dispatch(action) { state = historyUpdateState(state, action); app.syncState(state);
    }
  });
  return app.dom;
}
